...
function del(attid,cpage){
	$(".delWindow a:last").prop("href","javascript:toremove("+attid+","+cpage+")");
	maskResize();
...
function toremove(attid,cpage){
	$.get("ClubDeptServlet?opt=toremove","attid="+attid,function(data){
		if(data==1){
			...
			setTimeout(function(){
				location.href="ClubDeptServlet?opt=clubtoselect&cpage="+cpage;
			},2000);
		}else{
